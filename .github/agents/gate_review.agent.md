...
| # | 检查项 | 判定 | 说明 |
|---|--------|------|------|
| 8 | 是否有立项验证报告（pm_assistant 价值评估） | ✅/⚠️/❌ | |
| 9 | 竞品分析是否充分（≥3 个竞品） | ✅/⚠️/❌ | |
| 10 | 差异化优势是否明确 | ✅/⚠️/❌ | |
| 11 | 目标用户和痛点是否有数据支撑 | ✅/⚠️/❌ | |
...
| # | 检查项 | 判定 | 说明 |
|---|--------|------|------|
| 16 | 低保真原型是否覆盖所有 P0 页面 | ✅/⚠️/❌ | |
| 17 | 页面间跳转是否形成完整操作流程 | ✅/⚠️/❌ | |
| 18 | 核心用户路径是否可走通 | ✅/⚠️/❌ | |
...
| # | 检查项 | 判定 | 说明 |
|---|--------|------|------|
| 19 | PRD 文档头版本号与 §11 变更记录最新条目是否一致 | ✅/⚠️/❌ | |
| 20 | 状态字段是否为有效值（草稿/评审中/已批准/已替代） | ✅/⚠️/❌ | |
| 21 | 若为迭代版本，变更记录是否记录了本次变更内容 | ✅/⚠️/❌ | |
...
| 16 | 高可用和容灾方案是否有说明 | ✅/⚠️/❌ | |
| 17 | §7 测试架构是否定义了测试分层、框架选型和覆盖率目标 | ✅/⚠️/❌ | |
| 18 | §7.7 系统级测试方案是否覆盖跨模块关键业务场景和 NFR 验证追溯 | ✅/⚠️/❌ | |
| 19 | 若 PRD 为模块化，模块级架构中的接口依赖和回退/降级策略是否与主架构一致 | ✅/⚠️/❌ | |

#### D. 风险与成本（权重 15%）

| # | 检查项 | 判定 | 说明 |
|---|--------|------|------|
| 20 | 技术风险是否已识别并有应对方案 | ✅/⚠️/❌ | |
| 21 | 架构债务是否有演进路线 | ✅/⚠️/❌ | |
| 22 | 成本估算是否合理（如涉及付费服务） | ✅/⚠️/❌ | |

#### E. 版本管理（权重 5%）

| # | 检查项 | 判定 | 说明 |
|---|--------|------|------|
| 23 | 架构文档「关联 PRD」字段是否包含精确版本号（如 `prd-xxx.md v1.0.0`） | ✅/⚠️/❌ | |
| 24 | 关联的 PRD 版本号是否与实际 PRD 文档头版本一致 | ✅/⚠️/❌ | |
| 25 | 架构文档头版本号与 §12 变更记录最新条目是否一致 | ✅/⚠️/❌ | |
| 26 | 状态字段是否为有效值（草稿/评审中/已批准/已替代） | ✅/⚠️/❌ | |
| 27 | 若 PRD 为模块化，模块级架构文档头是否正确关联 Module PRD 与主架构版本 | ✅/⚠️/❌ | |
