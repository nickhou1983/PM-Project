...
> **触发条件**：当 PRD 采用模块化结构（`docs/prd-{项目名}/modules/` 目录下存在 Module PRD 文件）时，支持按模块生成独立的架构设计文档。
>
> **何时使用**：
> - 用户明确要求「为某个模块生成架构设计」
> - 用户要求「按模块拆分架构文档」
> - 在完成主架构文档后，需要对特定模块做深入设计

**输入**：
- Module PRD 文档路径（如 `docs/prd-{项目名}/modules/prd-{module_en_slug}.md`）
- 可选：主架构文档作为上下文参考

**流程**：

1. **读取 Module PRD** — 提取模块功能需求、用户故事、验收标准、依赖关系
2. **读取主架构文档**（如存在） — 获取全局技术栈选型、架构风格、部署方案作为基线约束
3. **设计模块级架构** — 聚焦该模块的：
   - **数据模型**：该模块涉及的数据库表和字段、ER 图（Mermaid）
   - **API 设计**：该模块对应的 RESTful 接口列表（路径、方法、请求/响应）
   - **服务设计**：该模块的核心服务/组件职责划分
   - **前端组件**：页面路由、组件结构、状态管理
   - **模块间接口**：与上下游模块的数据交互协议
4. **输出模块级架构文档** — 写入 `docs/prd-{项目名}/architecture-{项目名}-{module_en_slug}.md`
   - 文档头「关联 Module PRD」字段包含精确版本号
   - 文档头「关联主架构文档」字段引用主架构文档路径和版本
5. **更新 Module PRD 的技术参考** — 回填 Module PRD §5.3 中的占位符：
   - 将 `{待架构设计后补充}` 替换为实际的数据模型、API 端点、前端组件摘要
   - 更新模块级架构文档链接

**输出文件**：

```text
docs/prd-{项目名}/architecture-{项目名}-{module_en_slug}.md
```

**批量模式**：当用户要求「为所有模块生成架构设计」时，遍历 `modules/` 目录下的所有 Module PRD，逐一执行上述流程。
