...
6. **MVP 规模校准** — 根据 PRD 目标用户量和业务定位，确定架构复杂度等级，**避免过度设计**：
   ...
   | 规模等级 | 判定条件 | 推荐架构风格 | 约束 |
   |---------|---------|------------|------|
   | **L1（轻量级）** | 目标 DAU ≤ 1K 或 MVP 验证阶段 或 功能模块 ≤ 3 | 单体应用（Monolith） | 禁用微服务、K8s；优先 SQLite/PostgreSQL 单库；部署用单容器或 PaaS |
   | **L2（标准级）** | 目标 DAU 1K-50K 且功能模块 4-8 | 模块化单体（Modular Monolith） | 可用消息队列但不拆服务；单库或读写分离；部署用 Docker Compose 或托管服务 |
   | **L3（复杂级）** | 目标 DAU > 50K 或 功能模块 > 8 或 有明确的多团队并行开发需求 | 微服务（Microservices） | 可使用 K8s、服务网格、分库分表；需论证拆分必要性 |
