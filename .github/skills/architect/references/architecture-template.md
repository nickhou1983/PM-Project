...
> 本架构文档为主文档（总纲）。当采用模块化 PRD 时，本文档聚焦跨模块架构、共享能力、部署、安全与演进策略；各模块的详细设计位于独立模块级架构文档中。当采用分文档模式时，前端、后端、数据库的详细设计位于独立子文档中，主文档保留各章节的摘要和导航链接。

| 文档 | 路径 | 说明 | 模式 |
| ---- | ---- | ---- | ---- |
| **主架构文档（本文档）** | `architecture-{项目名}.md` | 系统整体架构、技术栈选型、部署方案、非功能需求 | 始终生成 |
| 模块级架构文档 | `architecture-{项目名}-{module_en_slug}.md` | 单个模块的数据模型、API、组件与依赖接口 | 模块化 PRD |
...
> **模块化模式**：本文档保留跨模块设计与模块导航，各模块的详细设计写入 `architecture-{项目名}-{module_en_slug}.md`。
...
> 若 PRD 采用模块化结构，本节优先描述跨模块边界；各模块内部边界下沉到对应的模块级架构文档。

### 1.5 需求追溯矩阵

> 将 PRD 中的 P0/P1/P2 功能需求映射到架构模块和 API，确保所有需求在架构中有迹可循。若 PRD 采用模块化结构，建议按 `module_en_slug` 分组列出，并在备注中引用对应模块级架构文档。
...
| --------- | ---- | -------- | ---- | ---------------- |
| {模块 1} | {职责描述} | {功能列表} | {依赖的其他模块} | `architecture-{项目名}-{module_en_slug}.md` |
| {模块 2} | {职责描述} | {功能列表} | {依赖的其他模块} | `architecture-{项目名}-{module_en_slug}.md` |
| {模块 3} | {职责描述} | {功能列表} | {依赖的其他模块} | `architecture-{项目名}-{module_en_slug}.md` |

> 模块化模式下，本节仅保留模块级摘要；详细数据模型、API 和组件设计见各模块级架构文档。
...
> 本节定义项目的整体测试策略、框架选型、分层目标和关键测试方案，确保从架构层面保障质量。各模块的具体测试场景详见对应 Module PRD §4 测试要点。
...
| 测试层级 | 占比目标 | 职责 | 执行频率 |
| -------- | -------- | ---- | -------- |
| **单元测试** | {如 60%-70%} | 验证函数/方法/组件的独立行为 | 每次提交 |
| **集成测试** | {如 20%-25%} | 验证模块间交互、API 契约、数据库操作 | 每次 PR |
| **E2E 测试** | {如 10%-15%} | 验证端到端用户流程和关键业务场景 | 每次部署 Staging |
...
| ---- | ------ | ---- |
| 行覆盖率（Line） | ≥ {X}% | {整体最低线} |
| 分支覆盖率（Branch） | ≥ {X}% | {条件分支覆盖} |
| P0 功能覆盖率 | 100% | P0 功能必须有完整测试用例覆盖 |
| API 接口覆盖率 | ≥ {X}% | {所有公开 API 端点} |

### 7.2 测试框架选型

| 测试类型 | 框架/工具 | 选型理由 | 备选方案 |
| -------- | --------- | -------- | -------- |
| 前端单元测试 | {如 Vitest / Jest} | {理由} | {备选} |
| 前端组件测试 | {如 Testing Library / Enzyme} | {理由} | {备选} |
| 后端单元测试 | {如 pytest / JUnit / Go testing} | {理由} | {备选} |
| API 集成测试 | {如 Supertest / REST Assured / httpx} | {理由} | {备选} |
| E2E 测试 | {如 Playwright / Cypress} | {理由} | {备选} |
| 性能测试 | {如 k6 / Artillery / JMeter} | {理由} | {备选} |
| 安全测试 | {如 OWASP ZAP / Snyk / Trivy} | {理由} | {备选} |
| Mock/Stub | {如 MSW / WireMock / unittest.mock} | {理由} | {备选} |

### 7.3 测试环境与数据策略

| 环境 | 测试类型 | 数据策略 | 说明 |
| ---- | -------- | -------- | ---- |
| 本地开发 | 单元测试 + 组件测试 | Mock 数据 / 内存数据库 | {说明} |
| CI 环境 | 单元 + 集成测试 | 测试容器（Testcontainers） / Seed 数据 | {说明} |
| Staging | 集成 + E2E 测试 | 脱敏生产数据 / Fixtures | {说明} |
| 生产 | 冒烟测试 + 监控 | 真实数据（只读验证） | {说明} |

**测试数据管理**：

- **数据生成**：{Faker / Factory / 自定义 Seed 脚本}
- **数据隔离**：{每次测试独立事务回滚 / 独立数据库 / 命名空间隔离}
- **敏感数据**：{脱敏规则 / 禁止使用真实 PII}

### 7.4 性能测试方案

| 测试场景 | 工具 | 目标指标 | 触发条件 |
| -------- | ---- | -------- | -------- |
| 基准性能测试 | {k6 / Artillery} | {QPS / 响应时间 P95/P99} | {每次 Release 前} |
| 压力测试 | {k6 / JMeter} | {系统崩溃点 / 最大并发数} | {重大版本发布前} |
| 容量规划验证 | {k6 / Locust} | {目标 QPS 下资源使用率} | {架构变更后} |

### 7.5 安全测试方案

| 测试项 | 工具/方法 | 频率 | 覆盖范围 |
| ------ | --------- | ---- | -------- |
| SAST（静态分析） | {如 SonarQube / Semgrep / CodeQL} | 每次 PR | 源代码漏洞扫描 |
| DAST（动态分析） | {如 OWASP ZAP} | 每次部署 Staging | 运行时漏洞扫描 |
| 依赖漏洞扫描 | {如 Snyk / Dependabot / Trivy} | 每日 | 第三方依赖 CVE |
| 容器镜像扫描 | {如 Trivy / Grype} | 每次构建镜像 | 基础镜像安全 |
| API 安全测试 | {如 OWASP ZAP API Scan} | 每次 Release 前 | 认证/鉴权/注入/越权 |

### 7.6 CI/CD 中的测试集成

> 与 §6.3 CI/CD 流水线对应，定义每个阶段执行的测试及质量门禁。

| 流水线阶段 | 执行的测试 | 质量门禁 | 失败处理 |
| ---------- | ---------- | -------- | -------- |
| 代码提交 | Lint + 格式检查 | 零 error | 阻断合并 |
| PR 构建 | 单元测试 + 集成测试 | 覆盖率 ≥ {X}%，零失败 | 阻断合并 |
| 合并主分支 | 全量单元 + 集成测试 | 覆盖率 ≥ {X}%，零失败 | 阻断部署 |
| 部署 Staging | E2E 测试 + 安全扫描 | 关键路径零失败，无高危漏洞 | 阻断生产部署 |
| 部署 Production | 冒烟测试 | 核心功能可用 | 触发自动回滚 |

### 7.7 系统级端到端测试方案

> 定义跨模块的端到端业务场景测试和 NFR 验证追溯，确保系统级质量。

**跨模块业务场景**：

| 场景名 | 覆盖模块 | 关键数据流 | 测试类型 | 优先级 |
| ------ | -------- | ---------- | -------- | ------ |
| {如用户注册→创建项目→邀请成员} | {模块 A, 模块 B, 模块 C} | {用户数据→项目数据→权限数据} | E2E | P0 |
| {如数据导入→处理→导出报告} | {模块 X, 模块 Y} | {原始数据→处理结果→报告文件} | E2E | P0 |

**NFR 验证追溯表**：

> 将 PRD §5 非功能需求的每项指标追溯到架构中的具体验证方案，确保 V-Model 左右对称。

| PRD §5 NFR 类别 | PRD 指标 | 架构验证工具 | 验证时机 | 通过标准 | 关联架构章节 |
| --------------- | -------- | ------------ | -------- | -------- | ------------ |
| 性能 | {如首屏 ≤2s} | {如 Lighthouse + k6} | {如每次 Release 前 + Staging 部署后} | {如 P95 ≤ 2s} | §8.1 性能设计 |
| 可用性 | {如 SLA ≥99.9%} | {如 Chaos Engineering + 监控} | {如每季度故障注入演练} | {如月度 uptime ≥99.9%} | §8.2 高可用设计 |
| 安全 | {如 TLS 1.3} | {如 OWASP ZAP + Trivy} | {如每次部署 Staging} | {如零高危漏洞} | §9 安全设计 |
| 兼容性 | {如主流浏览器} | {如 BrowserStack / Playwright multi-browser} | {如每次 Release 前} | {如所有目标浏览器零关键缺陷} | §3.4 前端架构 |
| 可扩展性 | {如 1000 QPS} | {如 k6 容量测试} | {如重大版本发布前} | {如目标 QPS 下 CPU ≤70%} | §8.3 可扩展性设计 |
| 数据合规 | {如 GDPR} | {如合规审计 + 数据脱敏验证} | {如上线前 + 每季度} | {如审计零不合规项} | §9.4 合规要求 |
