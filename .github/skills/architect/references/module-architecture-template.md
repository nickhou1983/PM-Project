...
| ---------- | ------------ | --------------- | --------- | ---------- | -------- | ---- |
| {当前模块} | {依赖模块} | `{API}` / {事件} | {方式} | P0/P1/P2 | {契约测试 / Mock / 集成测试} | {说明} |

> **测试优先级**：P0 接口必须有集成测试和契约测试；P1 接口至少有集成测试；P2 接口可用 Mock 替代。
> **测试策略**：契约测试验证接口格式不变；Mock 用于隔离测试依赖方不可用的场景；集成测试验证真实调用链路。
...
> 定义本模块与上下游模块之间的接口测试方案，确保模块间集成质量。

| 接口 | 对端模块 | 契约工具 | Mock 策略 | 关键测试场景 | 关联 Module PRD TC 编号 |
| ---- | -------- | -------- | --------- | ------------ | ---------------------- |
| `{API endpoint}` | {对端模块} | {如 Pact / OpenAPI Diff / 手动} | {如 MSW / WireMock / 内存 Mock} | {如正常响应 + 超时 + 降级} | TC-{module}-{NNN} |
| `{Event / Message}` | {对端模块} | {如 Schema Registry / JSON Schema} | {如 Fake Producer} | {如消息格式变更 + 消费失败重试} | TC-{module}-{NNN} |
