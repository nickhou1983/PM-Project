...
3. 生成完成后，将所有 Issue 以摘要形式展示给用户确认：
   ...
   ```text
   即将创建 {M} 个模块 Issue + {N} 个子 Issue：

   模块 1: [{项目名}] {模块1} — 功能需求 (P0, {X}个功能点)
     ├── [{项目名}] {功能点1} (P0)
     ├── [{项目名}] {功能点2} (P0)
     └── [{项目名}] {功能点3} (P1)

   模块 2: [{项目名}] {模块2} — 功能需求 (P1, {Y}个功能点)
     ├── [{项目名}] {功能点4} (P1)
     └── [{项目名}] {功能点5} (P1)
   ...
...
6. 创建完成后输出结果摘要：
   ...
   | # | Issue 标题 | 编号 | 类型 | 优先级 | 父 Issue | 状态 |
   | - | ---------- | ---- | ---- | ------ | -------- | ---- |
   | 1 | [VideoPrompt] Prompt 生成 — 功能需求 | #12 | Epic | P0 | — | ✅ 已创建 |
   | 2 | [VideoPrompt] 智能 Prompt 生成 | #13 | Task | P0 | #12 | ✅ 已创建 |
   | 3 | [VideoPrompt] 视觉风格选择 | #14 | Task | P0 | #12 | ✅ 已创建 |
   | 4 | [VideoPrompt] 视频参数配置 | #15 | Task | P0 | #12 | ✅ 已创建 |
