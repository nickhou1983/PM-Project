...
description: "沉浸式高端前端 UI 设计 Skill。为 GitHub Copilot 提供 Awwwards 级别的高保真 Web 体验构建指南，涵盖高级动效、排版系统和架构工艺。触发条件：(1) 构建高端 Landing Page，(2) 创建沉浸式 UI/交互组件，(3) 制作 Awwwards 风格界面，(4) 需要动效驱动的前端开发（GSAP/Framer Motion），(5) 高性能动画组件，(6) premium frontend / immersive UI / 高保真前端。"
---
